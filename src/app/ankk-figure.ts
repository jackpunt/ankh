...
export class Figure extends Meeple {
  ...
  override isLegalTarget(hex: AnkhHex, ctx?: DragContext) { // Police
    ...
    if (this.isStable(this.hex)) {
      if (Meeple.allMeeples.filter(m => m.hex?.isOnMap).length < 4) return true; // Test Hack!
...
export class Satet extends Guardian1 {
  // static override makeSource(player: Player, hex: Hex2, n = 2) {
  //   return Meeple.makeSource0(UnitSource<Satet>, Satet, player, hex, n);
  // }
}
...
export class Androsphinx extends Guardian3 {
  constructor(Aname: string, player?: Player) {
    super(`Andro\nsphinx-${Tile.allTiles.length}`, player);
    this.nameText.y -= this.nameText.getMeasuredHeight() / 3; // not clear why 3, rather than 2
  }
}
